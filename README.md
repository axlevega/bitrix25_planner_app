...
### CRON: автосинхронизация по интервалу

Синхронизация с Bitrix24 по расписанию (интервал задаётся в настройках интеграции, минуты). Используется **инкрементальный** режим (обновляются только изменённые с прошлой синхронизации задачи и пользователи).

- **CLI:** запускать каждые 1–5 минут, например:  
  `*/5 * * * * php /path/to/backend/scripts/cron_sync.php`
- **HTTP:** если доступ к CLI недоступен:  
  `GET /api/cron/sync?key=SECRET`  
  В `.env` задать `CRON_SECRET=ваш_секретный_ключ`; вызывать по cron, например:  
  `*/5 * * * * curl -s "https://ваш-домен.ru/api/cron/sync?key=ВАШ_CRON_SECRET"`

Перед первым запуском выполнить хотя бы одну полную синхронизацию вручную (Настройки → Синхронизация).
